...
#First the return values must be selected, which are the project titles and the sums each project has raised thus far. Then the first table is declared with the FROM table_name and thenn the type of JOIN METHOD to be used with the second_table. The last line declares how the two tables will connect, in this case the connection comes through the project id. The question is how to sum based on a project name.
...
def selects_the_user_name_age_and_pledge_amount_for_all_pledges
  "SELECT users.name, users.age, SUM(pledges.amount) FROM pledges LEFT OUTER JOIN users ON users.id = pledges.user_id GROUP BY(users.name);"
end
